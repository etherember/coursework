...
def unenroll_class(request, classroom_id):
    if request.method == 'POST':
        ...
        student_mapping = Students.objects.filter(student_id=request.user, classroom_id=classroom)
        if student_mapping.exists():
            student_mapping.delete()
            return redirect('home')
    return render(request, 'base/unenroll_class.html', {'classroom_id': classroom_id})
...
def join_class_request(request):
    if request.POST.get('action') == 'post':
        ...
        try:
            ...
            if student.exists():
                # User is already registered in the class
                return JsonResponse({'status': 'FAIL', 'message': 'You are already registered in this class.'})
        except Classrooms.DoesNotExist:
            return JsonResponse({'status': 'FAIL', 'message': 'Invalid class code.'})
