...
def join_class_request(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            class_code = form.cleaned_data['code']
            try:
                classroom = Classrooms.objects.get(class_code=class_code)
                # Check if the user is already registered in the class
                if Students.objects.filter(student_id=request.user, classroom_id=classroom).exists():
                    messages.warning(request, 'You are already registered in this class.')
                ...
                return redirect('join_class_request')
    else:
        form = JoinClassForm()
    return render(request, 'classroom/joinclass.html', {'form': form})